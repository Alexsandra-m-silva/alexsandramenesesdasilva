class TemplateEngine_renderData {
            ...
            // Example usage
            const tEngine = new TemplateEngine_renderData("./Education_template.html");
            const data = {
                pageTitle: "", // filled from data.json
                topics: [], // filled from data.json
                navTitle: "", // filled from navBar.json
                footer: [], // filled from data.json
                coverImages: [], // filled from data.json
            ...
            .then(pageData => {
                ...
                data.topics = pageData.topics;
                data.coverImages = pageData.coverImages;
                data.footer = pageData.footer;
            });
            ...
            .then(() => {
                ...
                // Set columns based on topics count, max 6
                const columns = Math.min(data.topics.length, 6);
                const topicsContainer = document.querySelector('.container-nav-topics');
                if (topicsContainer && window.innerWidth < 600) {
                    topicsContainer.style.gridTemplateColumns = "repeat(1, 1fr)";
                }
                if (topicsContainer && window.innerWidth >= 600) {
                    topicsContainer.style.gridTemplateColumns = `repeat(${columns}, 1fr)`;
                }
            });
            console.log(tEngine);
