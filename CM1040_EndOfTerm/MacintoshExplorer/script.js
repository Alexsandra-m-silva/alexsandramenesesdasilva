	...
	if (window.innerWidth > 699) {
		...
		// If the user is on small devices
		{
			setTimeout(function () {
				...
				errorMsg.innerHTML =
					"Sorry, a system error occured. <br> macOs deosn't work on small devices.";
...
class Window {
    ...
    */
	...
	resizeWindow() {
		...
		this.resizeBtn.onclick = (e) => {
			this.window.classList.toggle("anim-resize");
		};
	}
	...
	run() {
		...
		this.windowDraggable
			? this.drag(this.window, this.window)
			: this.drag(this.window, this.windowHeader);
		setInterval(() => {
			this.windowDraggable
				? this.drag(this.window, this.window, true)
				: this.window.offsetTop < 200 || this.window.offsetLeft < 0
				? this.drag(this.window, this.window, true)
				: this.drag(this.window, this.windowHeader, true);
		}, 100);
		this.window ? this.isActive() : {};
		this.window ? this.showWindow() : {};
		this.closeBtn
			? this.closeWindowUsingBtn()
			: this.closeWindowUsingBackground();
	}
}
	...
	if (drawMode === "pen") {
        ...
        */
		for (let i = 30; i > 0; i--) {
			...
			ctx.rect(
				lastX + Math.random() * parseInt(ctx.lineWidth) * 2 - 10,
				lastY + Math.random() * parseInt(ctx.lineWidth) * 2 - 10,
				1,
				1
			);
			ctx.fill();
...
(function () {
	...
	var mainLoop = () => {
		...
		if (
			snake[0].x < 0 ||
			snake[0].x == snakeCanvas.width / 10 ||
			snake[0].y < 0 ||
			snake[0].y == snakeCanvas.height / 10
		) {
			showScreen(2);
...
function digitCount(value, afterDecimal = true) {
	...
	if (afterDecimal) {
		...
		return value.toString().split(".").join("").length;
	}
}
// ############
...
resultbtn.addEventListener("click", (e) => {
	...
	while (multiply != -1) {
		numbersArr.splice(
			multiply,
			2,
			numbersArr[multiply] * numbersArr[multiply + 1]
		);
	...
	while (subtract != -1) {
		numbersArr.splice(
			subtract,
			2,
			numbersArr[subtract] - numbersArr[subtract + 1]
		);
	...
	while (add != -1) {
		...
		numbersArr.splice(
			add,
			2,
			parseFloat(numbersArr[add]) + parseFloat(numbersArr[add + 1])
		);
	...
	digitCount(numbersArr[0]) > 3
		? (display.innerHTML = numbersArr[0].toFixed(3))
		: (display.innerHTML = numbersArr[0]);
